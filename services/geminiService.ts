...
export const analyzeChart = async (input: string | BirthData): Promise<string> => {
  ...
  const locationLabel = langCode === 'es' ? 'Ubicación' : 'Location';
  ...
  const baseSystemPrompt = `
    ...
    DO NOT include technical labels like "H1" or "H2" in the text, just use the symbols.
    IMPORTANT: Write everything in ${languageName}.
